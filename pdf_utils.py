import os
import time
from pikepdf import Name, Object, Page, Pdf, Rectangle, Stream
import pikepdf
...
def add_watermark(path: str, saving_path: str):
    pdf = Pdf.open(path)
    water_mark=make_pdf_from_string("Hello this is water mark i want you too add ")
...
def put_password(password: str, path: str, saving_path: str, return_time=False) -> str | None:
    ...
    pdf.save(get_saving_pdf_name(path, saving_path),
             encryption=pikepdf.Encryption(owner=password, user=password, R=R))
    pdf.close()
    if return_time:
